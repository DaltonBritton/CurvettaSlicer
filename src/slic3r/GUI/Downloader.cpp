...
void Downloader::start_download(const std::string& full_url)
{
	...
	boost::regex re(R"(^(curvettaslicer|orcaslicer|prusaslicer|bambustudio|cura):\/\/open[\/]?\?file=)", boost::regbase::icase);
    ...
    if (is_bambustudio_open(full_url) || (is_curvetta_open(full_url) && is_makerworld_link(full_url)))
