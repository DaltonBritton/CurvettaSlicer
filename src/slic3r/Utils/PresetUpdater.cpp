    ...
    for (mz_uint i = 0; i < num_entries; ++i)
    {
        if (mz_zip_reader_file_stat(&archive, i, &stat))
        {
            ...
            if (stat.m_is_directory) {
                ...
                BOOST_LOG_TRIVIAL(warning) << "[Curvetta Updater]Unzip: invalid size for file "<<stat.m_filename;
                continue;
            }
            try
            {
                ...
                if (!res) {
                    BOOST_LOG_TRIVIAL(error) << "[Curvetta Updater]extract file "<<stat.m_filename<<" to dest "<<dest_file<<" failed";
                    close_zip_reader(&archive);
                    return res;
                }
                BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]successfully extract file " << stat.m_file_index << " to "<<dest_file;
            }
            catch (const std::exception& e)
            {
                // ensure the zip archive is closed and rethrow the exception
                close_zip_reader(&archive);
                BOOST_LOG_TRIVIAL(error) << "[Curvetta Updater]Archive read exception:"<<e.what();
                return false;
            }
        }
        else {
            BOOST_LOG_TRIVIAL(warning) << "[Curvetta Updater]Unzip: read file stat failed";
        }
    }
    close_zip_reader(&archive);
...
void PresetUpdater::priv::prune_tmps() const
{
    for (auto &dir_entry : boost::filesystem::directory_iterator(cache_path))
		if (is_plain_file(dir_entry) && dir_entry.path().extension() == TMP_EXTENSION) {
			BOOST_LOG_TRIVIAL(debug) << "[Curvetta Updater]remove old cached files: " << dir_entry.path().string();
...
void PresetUpdater::priv::sync_resources(std::string http_url, std::map<std::string, Resource> &resources, bool check_patch, std::string current_version_str, std::string changelog_file)
{
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("[Curvetta Updater]: sync_resources get preferred setting version for app version %1%, url: %2%, current_version_str %3%, check_patch %4%")%SLIC3R_APP_NAME%http_url%current_version_str%check_patch;
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("[Curvetta Updater]: sync_resources request_url: %1%")%url;
    http.on_progress([this](Slic3r::Http::Progress, bool &cancel_http) {
        ...
        .on_complete([this, &resource_list, resources](std::string body, unsigned) {
            try {
                BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]: request_resources, body=" << body;
                        ...
                        for (auto iter = resource.begin(); iter != resource.end(); iter++) {
                            ...
                            for (auto sub_iter = iter.value().begin(); sub_iter != iter.value().end(); sub_iter++) {
                                if (boost::iequals(sub_iter.key(), "type")) {
                                    ...
                                    BOOST_LOG_TRIVIAL(trace) << "[Curvetta Updater]: get version of settings's type, " << sub_iter.value();
                            ...
                            BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]: get type " << resource << ", version " << version << ", url " << url<<", force_update "<<force_upgrade;

                            resource_list.emplace(resource, Resource{version, description, url, force_upgrade});
                        }
                    }
                } else {
                    BOOST_LOG_TRIVIAL(error) << "[Curvetta Updater]: get version of settings failed, body=" << body;
                }
            } catch (std::exception &e) {
                BOOST_LOG_TRIVIAL(error) << (boost::format("[Curvetta Updater]: get version of settings failed, exception=%1% body=%2%") % e.what() % body).str();
            } catch (...) {
                BOOST_LOG_TRIVIAL(error) << "[Curvetta Updater]: get version of settings failed, body=" << body;
            }
        })
        .on_error([&](std::string body, std::string error, unsigned status) {
            BOOST_LOG_TRIVIAL(error) << boost::format("[Curvetta Updater]: status=%1%, error=%2%, body=%3%") % status % error % body;
        })
        .perform_sync();
    ...
    for (auto & resource_it : resources) {
        ...
        if (resource_update == resource_list.end()) {
            BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]Vendor " << resource_name << " can not get setting versions online";
            continue;
        }
        ...
        if (version_match && check_patch) {
            ...
            if (online_cc_patch != current_cc_patch) {
                version_match = false;
                BOOST_LOG_TRIVIAL(warning) << boost::format("[Curvetta Updater]: online patch CC not match: online_cc_patch=%1%, current_cc_patch=%2%") % online_cc_patch % current_cc_patch;
            }
        }
        if (version_match && (current_version < online_version)) {
            ...
            BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]Downloading resource: " << resource_name << ", version " << online_version.to_string();
            if (!get_file(online_url, cache_file_path)) {
                BOOST_LOG_TRIVIAL(warning) << "[Curvetta Updater]download resource " << resource_name << " failed, url: " << online_url;
                continue;
            }
            if (cancel) { return; }
            ...
            if (resource.sub_caches.empty()) {
                if (fs::exists(cache_path)) {
                    ...
                    BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]remove cache path " << cache_path.string();
                }
            } else {
                for (auto sub : resource.sub_caches) {
                    if (fs::exists(cache_path / sub)) {
                        fs::remove_all(cache_path / sub);
                        BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]remove cache path " << (cache_path / sub).string();
                    }
                }
            }
            // extract the file downloaded
            BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]start to unzip the downloaded file " << cache_file_path << " to "<<cache_path;
            fs::create_directories(cache_path);
            if (!extract_file(cache_file_path, cache_path)) {
                BOOST_LOG_TRIVIAL(warning) << "[Curvetta Updater]extract resource " << resource_it.first << " failed, path: " << cache_file_path;
                continue;
            }
            BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]finished unzip the downloaded file " << cache_file_path;
            ...
            BOOST_LOG_TRIVIAL(warning) << boost::format("[Curvetta Updater]: online version=%1%, current_version=%2%, no need to download") % online_version.to_string() % current_version.to_string();
        }
    }
}
...
void PresetUpdater::priv::sync_config()
{
    ...
    if (fs::exists(cache_profile_update_file)) {
        try {
            ...
            BOOST_LOG_TRIVIAL(error) << "[Curvetta Updater]: failed to read profiles_update.json when sync_config: " << ex.what() << std::endl;
        } catch (...) {
            // catch any other errors (that we have no information about)
            BOOST_LOG_TRIVIAL(error) << "[Curvetta Updater]: unknown failure when reading profiles_update.json in sync_config" << std::endl;
        }
    }
    ...
    Http::get(profile_update_url)
        .on_error([cache_profile_path, cache_profile_update_file](std::string body, std::string error, unsigned http_status) {
            ...
            BOOST_LOG_TRIVIAL(info) << format("Error getting: `%1%`: HTTP %2%, %3%", "sync_config_curvetta", http_status, error);
        })
        .timeout_connect(5)
        .on_complete([this, asset_name, cache_profile_path, cache_profile_update_file](std::string body, unsigned http_status) {
            ...
            try {
                ...
                if (!(j.contains("message") && j["message"].get<std::string>() == "Not Found")) {
                    ...
                    if (assets.is_array()) {
                        for (auto asset : assets) {
                            ...
                            int         versionNumber = -1;
                            std::regex  regexPattern("curvettaslicer-profiles_ota_.*\\.([0-9]+)\\.zip$");
                            std::smatch matches;
                ...
                if (latest_update.ver > 0) {
                    ...
                    // extract the file downloaded
                    BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]start to unzip the downloaded file " << download_file;
                    if (!extract_file(download_file, cache_profile_path)) {
                        BOOST_LOG_TRIVIAL(warning) << "[Curvetta Updater]extract downloaded file"
                                                   << " failed, path: " << download_file;
                        return;
                    }
                    BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]finished unzip the downloaded file " << download_file;
...
void PresetUpdater::priv::sync_tooltip(std::string http_url, std::string language)
{
    try {
        ...
        BOOST_LOG_TRIVIAL(warning) << format("[Curvetta Updater] sync_tooltip: %1%", e.what());
    }
}
...
void PresetUpdater::priv::sync_plugins(std::string http_url, std::string plugin_version)
{
    ...
    try {
        ...
        BOOST_LOG_TRIVIAL(warning) << format("[Curvetta Updater] sync_plugins: %1%", e.what());
    }

    bool result = get_cached_plugins_version(cached_version, force_upgrade);
    if (result) {
        BOOST_LOG_TRIVIAL(info) << format("[Curvetta Updater] found new plugins: %1%, prompt to update, force_upgrade %2%", cached_version, force_upgrade);
        if (force_upgrade) {
...
void PresetUpdater::priv::sync_printer_config(std::string http_url)
{
    ...
    try {
        ...
        BOOST_LOG_TRIVIAL(warning) << format("[Curvetta Updater] sync_printer_config: %1%", e.what());
    }
    ...
    if (result) {
        BOOST_LOG_TRIVIAL(info) << format("[Curvetta Updater] found new printer config: %1%, prompt to update", cached_version);
...
{
    BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]:Checking whether the profile from resource is newer";
                ...
                if ( fs::exists(path_in_vendor)) {
                    if (enabled_vendors.find(vendor_name) != enabled_vendors.end()) {
                        ...
                        if (!version_match || (vendor_ver < resource_ver)) {
                            BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]:found vendor "<<vendor_name<<" newer version "<<resource_ver.to_string() <<" from resource, old version "<<vendor_ver.to_string();
...
Updates PresetUpdater::priv::get_printer_config_updates(bool update) const
{
    ...
    if (!curr_version.empty()) {
        ...
        if (!version_match || (curr_ver < resc_ver)) {
            BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]:found newer version " << resc_version << " from resource, old version " << curr_version;
        } else {
            return {};
        }
    }
...
Updates PresetUpdater::priv::get_config_updates(const Semver &old_slic3r_version) const
{
	...
	BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]:Checking for cached configuration updates...";
        ...
        if (is_json_file(file_path)) {
            ...
            if (( fs::exists(path_in_vendor))
                ...
                if (version_match && (vendor_ver < cache_ver)) {
                    BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]:need to update settings from " << vendor_ver.to_string()
...
bool PresetUpdater::priv::perform_updates(Updates &&updates, bool snapshot) const
{
    ...
    if (updates.incompats.size() > 0) {
        ...
        //		return false;
        //}
        BOOST_LOG_TRIVIAL(info) << format("[Curvetta Updater]:Deleting %1% incompatible bundles", updates.incompats.size());
        ...
        BOOST_LOG_TRIVIAL(info) << format("[Curvetta Updater]:Performing %1% updates", updates.updates.size());
...
PresetUpdater::UpdateResult PresetUpdater::config_update(const Semver& old_slic3r_version, UpdateParams params) const
{
    ...
    if (updates.updates.size() > 0) {
        ...
        if (force_update)
        {
            BOOST_LOG_TRIVIAL(info) << format("[Curvetta Updater]:Force updating will start, size %1% ", updates.updates.size());
            std::vector<std::string> bundles;
            for (const auto& update : updates.updates) {
                if (update.is_directory)
                    continue;
            ...
            if (!ret) {
                BOOST_LOG_TRIVIAL(warning) << format("[Curvetta Updater]:perform_updates failed");
                return R_INCOMPAT_EXIT;
            }

            ret = reload_configs_update_gui();
            if (!ret) {
                BOOST_LOG_TRIVIAL(warning) << format("[Curvetta Updater]:reload_configs_update_gui failed");
                return R_INCOMPAT_EXIT;
            }
            for(auto b : bundles){
        ...
        if (params == UpdateParams::SHOW_NOTIFICATION) {
            ...
            BOOST_LOG_TRIVIAL(info) << format("[Curvetta Updater]:Configuration package available. size %1%, need to confirm...", p->waiting_updates.updates.size());
            ...
            if (res == wxID_OK) {
                BOOST_LOG_TRIVIAL(debug) << "[Curvetta Updater]:selected yes to update";
                ...
                BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]:selected no for updating";
        ...
        BOOST_LOG_TRIVIAL(info) << "[Curvetta Updater]:No configuration updates available.";
    }
