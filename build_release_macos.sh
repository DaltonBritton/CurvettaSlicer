...
function build_universal() {
    ...
    mkdir -p "$PROJECT_BUILD_DIR/CurvettaSlicer"
    UNIVERSAL_APP="$PROJECT_BUILD_DIR/CurvettaSlicer/CurvettaSlicer.app"
    rm -rf "$UNIVERSAL_APP"
    cp -R "$PROJECT_DIR/build/arm64/CurvettaSlicer/CurvettaSlicer.app" "$UNIVERSAL_APP"

    # Get the binary path inside the .app bundle
    BINARY_PATH="Contents/MacOS/CurvettaSlicer"

    # Create universal binary using lipo
    lipo -create \
        "$PROJECT_DIR/build/x86_64/CurvettaSlicer/CurvettaSlicer.app/$BINARY_PATH" \
        "$PROJECT_DIR/build/arm64/CurvettaSlicer/CurvettaSlicer.app/$BINARY_PATH" \
